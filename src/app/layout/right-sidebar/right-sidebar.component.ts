  ...
  implements OnInit, AfterViewInit
{
  ...
  ngAfterViewInit() {
    ...
    if (this.localStorageService.get('font_family')) {
      const savedFont = this.localStorageService.get('font_family') as string;
      const option = this.fontOptions.find(o => o.name === savedFont);
      if (option) {
        this.selectedFont = option.name;
        this.document.documentElement.style.setProperty('--body-font-family', option.value);
      }
    } else {
      const configFont = this.config.layout.font_family || 'Roboto';
      const option = this.fontOptions.find(o => o.name === configFont);
      if (option) {
        this.selectedFont = option.name;
        this.document.documentElement.style.setProperty('--body-font-family', option.value);
      }
  ...
  selectFont(fontName: string) {
    const option = this.fontOptions.find(o => o.name === fontName);
    if (option) {
      this.selectedFont = option.name;
      this.document.documentElement.style.setProperty('--body-font-family', option.value);
      this.localStorageService.set('font_family', fontName);
    }
  }
